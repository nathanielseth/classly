...
import { signIn, signUp } from '@/lib/server/functions/auth-actions'
...
const ROLES: Array<{ id: Role; label: string; icon: typeof GraduationCap }> = [
        ...
        <div className="w-full max-w-md">
          ...
          <div className="p-1.5 bg-gray-100 rounded-xl flex items-center gap-1 mb-6 relative">
            <div
              ...
              style={{
                width: 'calc(50% - 6px)',
                left: authMode === 'login' ? '6px' : 'calc(50%)',
              }}
            />
            {(
              [
          ...
          {notice && (
            <div className="mb-4 p-3 rounded-lg bg-classly-green/10 text-classly-dark text-sm">
              {notice}
            </div>
          )}
          {error && (
          ...
          <form onSubmit={handleSubmit} className="w-full">
            ...
            <button
              type="submit"
              disabled={submitting}
              className="mt-6 w-full py-3 px-4 rounded-xl bg-classly-green hover:bg-classly-dark text-white font-bold shadow-lg shadow-classly-green/30 transition-all duration-200 transform hover:-translate-y-0.5 active:scale-[0.98] flex items-center justify-center gap-2 cursor-pointer disabled:opacity-60 disabled:hover:translate-y-0 disabled:cursor-not-allowed"
            >
              {submitting ? (
